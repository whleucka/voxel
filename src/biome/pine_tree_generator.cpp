...
void PineTreeGenerator::generate(Chunk &chunk, int x, int y, int z) {
  ...
  // Trunk
  for (int j = 1; j < h; j++) {
    chunk.at(x, y + j, z) = BlockType::PINE_LOG;
  }
    ...
    for (int dx = -radius; dx <= radius; dx++) {
      for (int dz = -radius; dz <= radius; dz++) {
        if (dx * dx + dz * dz <= radius * radius) {
          ...
          if (rand() % 6 != 0) {
            chunk.at(x + dx, yy, z + dz) = BlockType::PINE_LEAF;
          }
        }
      }
    }
  ...
  chunk.at(x, y + h, z) = BlockType::PINE_LEAF;
  chunk.at(x, y + h + 1, z) = BlockType::PINE_LEAF;
}
