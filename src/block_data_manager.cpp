...
glm::vec2 BlockDataManager::getUV(BlockType type, BlockFace face) const {
    ...
    std::string type_str;
    switch (type) {
        case BlockType::DIRT:        type_str = "dirt";        break;
        case BlockType::GRASS:       type_str = "grass";       break;
        case BlockType::STONE:       type_str = "stone";       break;
        case BlockType::SAND:        type_str = "sand";        break;
        case BlockType::COBBLESTONE: type_str = "cobblestone"; break;
        case BlockType::WATER:       type_str = "water";       break;
        case BlockType::SNOW:        type_str = "snow";        break;
        /* If you use bedrock anywhere, add it to your enum and handle it: */
        default:                     type_str = "dirt";        break;
    }

    if (!data.is_object()) return {0,0};
    auto itBlocks = data.find("blocks");
    if (itBlocks == data.end() || !itBlocks->is_object()) return {0,0};
    auto itType = itBlocks->find(type_str);
    if (itType == itBlocks->end() || !itType->is_object()) return {0,0};

    const auto& block_data = *itType;

    auto pick2 = [&](const char* k)->glm::vec2 {
        if (block_data.contains(k) && block_data[k].is_array() && block_data[k].size() >= 2) {
    ...
    if (block_data.contains("uv") && block_data["uv"].is_array() && block_data["uv"].size() >= 2) {
        return { block_data["uv"][0].get<float>(), block_data["uv"][1].get<float>() };
    }

    switch (face) {
        case BlockFace::TOP:    return pick2("uv_top");
        case BlockFace::BOTTOM: return pick2("uv_bottom");
        default:                return pick2("uv_side");
    }
}
