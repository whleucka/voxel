...
static inline bool isOpaque(BlockType t) { return !isAir(t) && !isTransparent(t); }

static inline void tileToUV(float tileX, float tileY,
    float atlasSizePx, float tileSizePx,
...
void GreedyMesher::build(const Chunk &chunk,
  ...
  opaque_textures.push_back(TextureManager::getInstance().getTexture("block_atlas"));
  transparent_textures.push_back(TextureManager::getInstance().getTexture("block_atlas"));
  ...
  for (int y = 0; y < Chunk::H; ++y) {
    for (int x = 0; x < Chunk::W; ++x) {
      for (int z = 0; z < Chunk::L; ++z) {
        ...
        auto faceUV = [&](BlockFace face, float &u0, float &v0, float &u1, float &v1) {
          const glm::vec2 tile = BlockDataManager::getInstance().getUV(bt, face);
        ...
        // +Z (FRONT)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::FRONT, u0,v0,u1,v1);
            glm::vec3 p[4]={{x, y, z+1},{x+1,y, z+1},{x+1,y+1,z+1},{x,y+1,z+1}};
        ...
        // -Z (BACK)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::BACK, u0,v0,u1,v1);
            glm::vec3 p[4]={{x+1,y,z},{x,y,z},{x,y+1,z},{x+1,y+1,z}};
            pushQuad(V, I, p, {0,0,-1}, u0,v0,u1,v1);
          }
        }
        // +X (RIGHT)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::RIGHT, u0,v0,u1,v1);
            glm::vec3 p[4]={{x+1,y,z},{x+1,y,z+1},{x+1,y+1,z+1},{x+1,y+1,z}};
        ...
        // -X (LEFT)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::LEFT, u0,v0,u1,v1);
            glm::vec3 p[4]={{x,y,z+1},{x,y,z},{x,y+1,z},{x,y+1,z+1}};
            pushQuad(V, I, p, {-1,0,0}, u0,v0,u1,v1);
          }
        }
        // +Y (TOP)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::TOP, u0,v0,u1,v1);
            glm::vec3 p[4]={{x, y+1, z+1},{x+1,y+1,z+1},{x+1,y+1,z},{x,y+1,z}};
        ...
        // -Y (BOTTOM)
        {
          ...
          const bool draw = isAir(nbt) || (isOpaque(bt) && isTransparent(nbt)) || (isTransparent(bt) && isOpaque(nbt));
          if (draw) {
            float u0,v0,u1,v1; faceUV(BlockFace::BOTTOM, u0,v0,u1,v1);
            glm::vec3 p[4]={{x, y, z},{x+1,y,z},{x+1,y,z+1},{x, y, z+1}};
            pushQuad(V, I, p, {0,-1,0}, u0,v0,u1,v1);
          }
        }
      }
