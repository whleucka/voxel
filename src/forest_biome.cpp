...
const int TREE_BORDER_THRESHOLD = 5;
...
static BlockType generateInternalBlock(int x, int y, int z, int world_x, int world_z) {
  const double stone_noise = glm::perlin(
      glm::vec3((world_x * Chunk::W + x) * 0.55, y * 0.25, (world_z * Chunk::L + z) * 0.25));
...
static BlockType generateTopBlock(int x, int y, int z, int world_x, int world_z) {
    const double snow_noise = glm::perlin(
        glm::vec2((world_x * Chunk::W + x) * 0.02, (world_z * Chunk::L + z) * 0.02));
...
void ForestBiome::spawnDecorations(Chunk& chunk) {
    for (int x = TREE_BORDER_THRESHOLD; x <= Chunk::W - 1 - TREE_BORDER_THRESHOLD; ++x) {
        for (int z = TREE_BORDER_THRESHOLD; z <= Chunk::L - 1 - TREE_BORDER_THRESHOLD; ++z) {
            int y = 0;
            ...
            if (y > 0 && chunk.getBlock(x, y, z) == BlockType::GRASS) {
                const double tree_noise = (glm::perlin(
                    glm::vec3((chunk.world_x * Chunk::W + x) * 0.8, y * 0.02, (chunk.world_z * Chunk::L + z) * 0.8)) * 0.5 + 0.5);

                if (tree_noise > 0.6) { // Increased threshold
                    // set trunk
                    int h = (rand() % 6) + 10;
                    for (int j = 1; j <= h; j++) {
                        ...
                        for (int dx = -radius; dx <= radius; dx++) {
                            for (int dz = -radius; dz <= radius; dz++) {
                                ...
                                if (dx * dx + dz * dz + dy * dy <= radius * radius + 1) {
                                    if (chunk.getBlock(x + dx, top + dy, z + dz) != BlockType::TREE)
                                        chunk.setBlock(x + dx, top + dy, z + dz, BlockType::TREE_LEAF);
                                }
                            }
                        }
                    }
